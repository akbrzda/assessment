...
async function deleteCourse(req, res, next) {
  try {
    ...
    if (existing.status === "published") {
      return res.status(409).json({ error: "Нельзя удалить опубликованный курс. Сначала переведите его в архив." });
    }
  ...
  try {
    ...
    if (!integrity.valid) {
      return res.status(422).json({
        error: "Курс не готов к публикации",
...
async function updateModule(req, res, next) {
  try {
    ...
    if (!moduleId) {
      return res.status(400).json({ error: "Некорректный идентификатор модуля" });
    }
