...
async function create(req, res, next) {
  try {
    ...
    const invitation = await invitationsService.createInvitation(payload, req.currentUser, req);
...
async function remove(req, res, next) {
  try {
    const invitationId = ensureInvitationId(req.params.id);
    await invitationsService.removeInvitation(invitationId, req.currentUser, req);
    res.status(204).send();
  } catch (error) {
    if (error.status) {
      return res.status(error.status).json(formatErrorPayload(error));
