...
> ⚠️ **Проблема**: тип `text` требует точного совпадения. Эталонные ответы не поддерживают нечёткое сравнение, что делает их малопригодными для реальных ответов на русском языке.
...
> ⚠️ **Проблема**: поддерживаются только типы `text`, `video`, `link`. Нет поддержки картинок, аудио и iframe (заявлено в `features.md`).
...
> ⚠️ При сохранении — **все вопросы удаляются и добавляются заново**:
...
| 7. Пользовательский UI                 | 6/7    | ⚠️ 85% (нет адаптива) |
